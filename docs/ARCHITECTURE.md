...
| Transcribe | `internal/transcribe` | Parakeet via sherpa-onnx (cgo), or a stub. Always local |
| Clean | `internal/cleanup` | Qwen via the bundled llama-server, or a hosted model via OpenRouter, same client |
| Inject | `internal/inject` | Clipboard write, paste, deferred restore |
| Overlay | `internal/overlay` | Cursor dot: red recording, blue processing, raw Win32 |

The engine runs five goroutines: the trigger's hook thread, a worker that starts and stops recording, the recorder's poll loop that drains the capture ring, and two output stages -- one transcribing, one cleaning and injecting -- joined by a channel. Splitting them means the tail the speaker just released is already being transcribed while the segment before it is still in the cleaner. Each stage is a single goroutine, so pieces still land in order.

The user picks the cleanup backend in settings. The engine keeps the local and cloud cleaners side by side and swaps the active one live, so a key can be entered or removed without a restart. Transcription has no such choice: local Parakeet or nothing.
...
- The first request processes the roughly 1,000-token system prompt, then the prefix caches and later requests reprocess a single token. On an RTX A4000 laptop that is about 0.2s for a short utterance and 0.8s for a long one, generation-bound at roughly 75 tok/s. 0type prewarms it on startup. A hosted model has no such cache to hit and pays a network round trip besides, which is why cloud cleanup measures slower than local on any hardware that can run the 4B at all.
- The system prompt lives in [`internal/cleanup/prompt.txt`](../internal/cleanup/prompt.txt).

## Live mode

By default 0type pastes as you speak. The recorder streams each captured chunk to `audio.Segmenter`, which gates 20 ms frames by energy against an adaptive noise floor and cuts a segment once speech has been heard and a pause of 0.5 s follows (at least 1 s per segment; past 7 s any 200 ms pause will do; 20 s cuts regardless). Those are latency settings, not quality ones: what the speaker waits through after letting go is the tail left since the last cut, so the soft maximum is what bounds it. At 20 s a continuous dictation left about 20 s of audio still to transcribe on release, roughly 1.4 s of work; at 7 s there is usually a few hundred milliseconds. Each segment is wrapped as WAV and queued to the same output worker as a whole recording would be, so pieces paste in order. On release, the worker gets the tail after the last cut, sliced from the full recording by the segmenter's byte offset; `Stop` flushes the last ring buffers through the stream callback first so that offset is exact.
...
The trade-off is that cutting this often, the cleaner usually sees a fragment -- one that begins mid-sentence as well as ending mid-sentence. The prompt covers both: stop where the speaker stopped, and keep the opening words that finish what `<previous>` left hanging. That second rule is load-bearing. Without it the 4B model silently dropped those words, reading them as part of the context it was told never to repeat, and whole phrases went missing. Turning live mode off restores paste-on-release.

## Recommended models

`internal/recommend` boils OpenRouter's rankings down to the picker under the cleanup model field. It reads the documented catalog (prices, the Artificial Analysis intelligence index) plus the JSON feed the rankings page itself loads (per-model p50 latency). It builds three lists: best value is intelligence per blended dollar, fastest is lowest p50 latency, smartest is the raw index; all three need real traffic, and the first two a score of at least 25. The result is cached in the data dir for a day; if a refresh fails, the cache or an embedded snapshot (`snapshot.json`, regenerated with `ZEROTYPE_WRITE_SNAPSHOT=1 go test ./internal/recommend`) is shown marked as offline. The rankings feeds are undocumented, so a refresh that finds one missing or reshaped fails as a whole and the previous copy stands. Rankings data is CC BY 4.0; the attribution line is shown in the window.
...
1. **Paste, don't type.** Inject saves the clipboard, writes the text, and sends Ctrl+V. Simulating keystrokes is slow and mangles Swedish characters and emoji. The restore runs on a timer once the pastes stop, not inline: the clipboard calls themselves cost well under a millisecond, but waiting for the target app to consume each paste cost 120 ms per segment on the output worker.
...
4. **Two output stages, each single-threaded.** Transcription and cleanup are separate goroutines joined by a channel, so they overlap; each staying single means two quick dictations still cannot fight over the clipboard or land out of order.
5. **Load models off the startup path.** Parakeet takes seconds to read into memory, and the cleanup server longer still. Both load in the background and swap in when ready, so the window and the trigger come up at once. Dictating before they land says so rather than failing silently or quietly skipping cleanup.
...
Cloud cleanup needs nothing downloaded: OpenRouter's chat completions speak the same shape as llama-server, so one client covers both local and hosted. There is no cloud transcription. It was the slowest path in the app -- a base64 WAV in a JSON body, uploaded and waited on -- and the only one that sent the speaker's audio anywhere, which is a poor trade against a 600 MB local model running at 14x realtime.
